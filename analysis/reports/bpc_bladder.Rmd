...
dft_timing_long %<>% 
  mutate(
    origin = case_when(
      str_detect(interval, "\\(from dx\\)$") ~ origin_levs[1],
      str_detect(interval, "\\(from first regimen\\)$") ~ origin_levs[2],
      str_detect(interval, "\\(from metastasis\\)$") ~ origin_levs[3],
    ),
    origin = factor(origin, levels = vec_origin_levs)
  )

gg_timing_all <-  plot_timing_intervals(
  dat = dft_timing_long
)
...
plot_timing_ecdf(dft_timing_long)
```
...
1. Time (years) from diagnosis to the first next generation sequencing test **report date**.
2. Time (years) from diagnosis to the start of the first regimen.
3. Time (years) from first regimen to the first NGS report date.
3. Time (years) from diagnosis to the first note of distant metastasis (some are diagnosed at Stage IV, implying a time of zero years).
4. Time (years) that the participant was followed for survival.  Follow-up ends with censoring or death.

These are violin plots, which show the density of observations.  Each individual observation is plotted in light opacity these density representations.  Summary statistics for each interval are printed on the rigth hand side.
...
- Add converted KPS scores to ECOG stats.
- Add time from first regimen to first genomic sequence to figure R1.3.
- Add swimmer plot.
- Fill out table of metastatic disease state.
