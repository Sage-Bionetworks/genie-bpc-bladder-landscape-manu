...
dft_img <- read_wrap("img.csv")
dft_med_onc <- read_wrap("med_onc.csv")
dft_path <- read_wrap("path.csv")
dft_reg <- read_wrap("reg.csv")
# dft_tm <- read_wrap("tm_level_dataset.csv")
dft_cpt <- read_wrap("cpt.csv")
...
read_wrap_dmet <- read_wrap <- function(p) {
  read_csv(file = here("data", 'dmet', p), show_col_types = F)
}

dft_pt_dmet <- read_wrap_dmet("pt_dmet.csv")
dft_ca_ind <- read_wrap_dmet("ca_ind_dmet.csv")
dft_reg_dmet_s <- read_wrap_dmet("reg_start_gte_dmet.csv")
```
...
**Placeholder.**  Please see the timing plots below, which answer most of the items we intended to put here.
...
dft_timing_long %<>% 
  mutate(
    origin = case_when(
      ...
      str_detect(interval, "\\(from metastasis\\)$") ~ vec_origin_levs[3],
    ),
...
4. Time (years) from diagnosis to the first note of distant metastasis (some are diagnosed at Stage IV, implying a time of zero years).
