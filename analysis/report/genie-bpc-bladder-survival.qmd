...
```{r}
dft_ca_ind_ddr <- readr::read_rds(
  here('data', 'survival', 'ddr_neoadj', 'ca_ind_ddr.rds')
)
```


### *R3.2.6 DDR neoadjuvant

In our August 2023 meeting we discussed the results above, and the fact that the trial that inspired the look into these specific DDR mutations was in an earlier stage setting than what we've looked at.  To start looking at this, we'll first figure out how big the overlap between the group with DDR mutations and neoadjuvant therapy is.

The following table is limited to people who have an oncogenic DDR mutation **at any time** (same list as above).  In the rows we're showing the results of the `ca_tx_pre_path_stage` variable.  The columns split the results by `stage_dx_iv` (see notes after the table).

```{r}
#| include: true
dft_ca_ind_ddr %>%
  tabyl(ca_tx_pre_path_stage, stage_dx_iv) %>%
  flextable(.) %>%
  autofit(.)
```
**Notes:**

- 33 people appear to be unambiguously receiving neoadjuvant treatment and have a DDR mutation at some time during followup.
- `ca_tx_pre_path_stage` note from the data guide: "Populated only if cancer was not diagnosed at stage IV ([ca_stage_iv] = 'No'."  We don't have that variable in the release, but its odd to me that that we have people who had neoadjuvant therapy but `stage_dx_iv = Stage IV`.  Chelsea Nichols is helping me look into this.
...
dft_neoadj_ddr %>% count(ca_clin_t_stage, sort = T)

dft_neoadj_ddr %<>% 
  mutate(
    naaccr_path_t_cd_filled = !is.na(naaccr_path_t_cd),
    ca_path_t_stage_filled = !is.na(ca_path_t_stage) & 
      !(ca_path_t_stage %in% c("Unknown", "Not Applicable")),
    naaccr_clin_t_cd_filled = !is.na(naaccr_clin_t_cd),
    ca_clin_t_stage_filled = !is.na(ca_clin_t_stage) & 
      !(ca_clin_t_stage %in% c("Unknown", "Not Applicable")),
    has_path = naaccr_path_t_cd_filled | ca_path_t_stage_filled,
    has_clin = naaccr_clin_t_cd_filled | ca_clin_t_stage_filled,
    has_both = has_clin & has_path
  )

dfp_staging_sum <- dft_neoadj_ddr %>%
  summarize(
    across(
      .cols = matches("^has_"),
      .fns = sum
...
The next step of this inquiry is looking at the pathological and clinical staging for these people to see if the two differ.  Here are the variables we're using to look at this:

- Pathological: If either `naaccr_path_t_cd` or `ca_path_t_stage` are not missing, we consider this completed ("has_path" below).
- Clinical:  If either `naaccr_clin_t_cd` or `ca_clin_t_stage` are not missing, we consider this completed ("has_clin" below).
...
So we could look at the difference in clinical and pathological staging in about 29 people.  There are two outstanding issues, the first is dealing with the timing of the DDR mutation (for example, if it's identified 10 years after staging it's hard to see the relevance).  The second is we will have to deal with the free-text in the staging.  Here is what the clinical and pathological staging variables look like for these 29 people (just to give an idea of the heterogeneity of codes):
...
**Observation:**  It's obvious that the coding style for both variables is dramatically different at each site.  We have work to do if we want to make sense of this.
