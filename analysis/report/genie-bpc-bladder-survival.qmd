...
dft_ddr_stage_diffs <- readr::read_rds(
  here('data', 'survival', 'ddr_neoadj', 'ddr_neoadj_stage_diffs.rds')
)
...
After discussing on Aug 26, 2024 we decided to look for people with a change in N stage between clinical and pathological staging.  For those with the same N stage, we will look at the difference in T stage.  Here are the cleaned up codes, where `_comb_` takes the tumor registry value if available, and the curated one otherwise:

```{r}
#| include: true

dft_ddr_stage_diffs %>%
  arrange(desc(abs(diff_n)), desc(abs(diff_t))) %>%
  flextable(.) %>%
  autofit(.)
```
...
- Obviously we have missing values after processing.  Examples are entries like "TpA" which tells us nothing.  About 1/3 of these samples can't even be confirmed to have the same N stage betweeen clinical and pathologic staging, and we're down to about 15 people if we want people with a usable T and N stage.
- The only N stage difference that exists is +3, which seems implausible, probably a typo from the clinical.
