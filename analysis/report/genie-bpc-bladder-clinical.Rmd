...
If we just focus on those cases that are a first cancer, it seems implausible that those with multiple cancers ("First") were more likely to receive zero lines of therapy associated with cancer #1, even though they had much longer average followup times.  One possible explanation is an error in the curation of the regimen data (if a regimen was taken but not assigned as possibly related to the first cancer, for example).  I think we can check thise more carefully if needed, but it will be a major tangent from this paper.

One other comment, more to the point of this section:  If we limit to only first and only cancers, the followup time distribution looks as we would expect it to:
