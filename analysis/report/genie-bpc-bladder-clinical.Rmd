...
```{r}
char_with_mg <- readr::read_rds(
  here('data', 'cohort', 'formatted_characteristics_with_main_genie.rds')
)
```

### R1.1.1c: Characteristics in main GENIE

Showing the columns that can also be assessed in main GENIE.  Every person in BPC is represented in main genie, so these columns are not distinct.  We have added the age at sequencing feature because age at dx does not get recorded in main GENIE.

```{r}
gt_char_with_mg <- char_with_mg %>%
...
gt_char_with_mg %>%
  ...
  save_as_pptx(., path = here('output', 'other', '2025_ss_char_with_mg.pptx'))
```
...
upset_met_loc <- dft_met_loc %>%
  select(record_id, ca_seq, met_loc, had_met) %>%
  pivot_wider(
    names_from = met_loc,
    values_from = had_met
  )  %>%
...
### R1.2.4d Lines of therapy - time stats
...
### *R1.2.4e Lines of therapy - people stats 
