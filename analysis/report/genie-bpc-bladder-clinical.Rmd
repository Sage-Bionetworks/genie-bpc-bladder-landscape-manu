...
dft_reg_met <- dft_reg_dmet_s %>%
  filter(str_detect(regimen_drugs, "Investigational", negate = T))
dft_reg_no_invest <- dft_reg %>%
  filter(str_detect(regimen_drugs, "Investigational", negate = T))
dft_reg_non_met <- anti_join(
  dft_reg_no_invest,
  dft_reg_dmet_s,
  by = c("record_id", "ca_seq", "regimen_number")
)

chk_reg_split <- nrow(dft_reg_no_invest) - 
  nrow(dft_reg_non_met) - 
  nrow(dft_reg_met)
if (!(chk_reg_split %in% 0)) {
  cli::cli_abort("Splitting the regimen dataset into met and non-met cases did not result in the expected number of rows.")
}

# For the metastatic version we will add in time variables from metastasis.
...
dfp_lot_met <- dft_lot_met %>%
  reshape_lot_stats(
    n_denom = nrow(dft_ca_ind_dmet)
  )
...
dfp_lot_met %<>% rename(` ` = Quantity)

```

### *R2.4a Lines of therapy - strategy

We define lines of therapy as a regimen of drugs started on or after the diagnosis of metastatic cancer.  This is similar to the concept of regimens as used in GENIE BPC with three important differences:
...
# Finding the GemCis and GemCarbo cases I want:
temp_ex1 <- dft_lot %>%
...
The following table addresses only drug regimens which were started on or after a diagnosis of metastasis (either de novo or after followup).  We are defining first line therapy ("First") as the first regimen following this event.  "Second" is the second regimen, etc.  For all timing variables we show the median (first quartile, third quartile).  The denominator shown for n is the number of participants who developed metastatic cancer in the cohort.
...
```{r}
dfp_lot_common_met <- dft_reg_met %>%
  get_lot_most_common(.) %>%
  display_lot_most_common(.)

dfp_lot_common_met %<>% rename(` ` = common)

```


### *R2.4b LoT, most common

The following table shows the most common first, second, third, regimen used in the metastatic setting.
...
dfp_lot_non_met <- dft_lot_non_met %>%
  reshape_lot_stats(.)
...
dfp_lot_non_met %<>% rename(` ` = Quantity)
```

### *R2.5a Pre-met regimens, stats

Here we describe regimens started before a diagnosis of metastasis (including participants who never developed metastasis).  Some participants are represented in both tables.  For example, a subject who developed metastasis at year 2 but had regimens during years 1 and 3 would appear in both.
...
dfp_lot_common_non_met <- dft_reg_non_met %>%
  get_lot_most_common(.) %>%
  display_lot_most_common(.)

dfp_lot_common_non_met %<>% rename(` ` = common)
```


### *R2.5b Pre-met regimens, most common

The following table shows the most common first, second, third, regimen used in the metastatic setting.

```{r, include = T}
dfp_lot_common_non_met %>%
  flextable %>%
  fontsize(., size = 8) %>%
  autofit(.)
```
