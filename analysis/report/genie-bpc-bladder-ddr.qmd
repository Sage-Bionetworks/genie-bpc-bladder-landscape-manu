...
```{r}
bundle_bpc_main_ddr <- readr::read_rds(
  here(dir_ddr_outcome, 'bundle_bpc_main_combined.rds')
)
```


### R4.9 - DDR as outcome (both)

A few figures that combine the previous two sections into one figure (in order of how much I like them)

```{r}
#| include: true
bundle_bpc_main_ddr$gg_ddr_out_both_panels[[1]]
```
...
```{r}
#| include: true
bundle_bpc_main_ddr$gg_ddr_out_both_multiple_lr[[1]]
```


```{r}
#| include: true
bundle_bpc_main_ddr$gg_ddr_out_both_jumble[[1]]
```
