...
# The main difference is we want ALL 2L therapies here, with flags for DDR
#   and flags for io.
# This script is highly redundant with the similar 1L script, really just 
#   the relevant regimens that change a bit.
...
# We made the decision that nivolumab didn't count as IO for this analysis.
# To make that very clear I'll redo the regimen categories a bit:
rel_reg %<>%
  # second part of this just ensures that the split goes how I want it to:
  arrange(regimen_cat, desc(str_detect(regimen_drugs, "brolizumab"))) %>%
  mutate(
    regimen_cat = case_when(
      regimen_cat %in% "Immunotherapy" & str_detect(regimen_drugs, "Atezo|Pembro") ~ "IO (atezo/pembro)",
      regimen_cat %in% "Immunotherapy" ~ "IO (other)",
      T ~ as.character(regimen_cat)
    ),
    regimen_cat = forcats::fct_inorder(regimen_cat)
  )
...
readr::write_rds(
  rel_reg,
  here(dir_out, 'met_ddr_surv_all_2L.rds')
)
...
# To make that very clear I'll redo the regimen categories a bit:
io_only <- rel_reg %>%
  filter(regimen_cat %in% "IO (atezo/pembro)")

readr::write_rds(
  io_only,
  here(dir_out, 'met_ddr_surv_atez_pem_2L.rds')
)
