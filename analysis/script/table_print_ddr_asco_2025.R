...
dft_cohort_ddr <- dft_cpt %>%
  select(sample_id = cpt_genie_sample_id, record_id, ca_seq) %>%
...
dft_onco_ddr_flags <- dft_onco_ddr %>%
  ...
  summarize(
    ddr_before_pm_reg = sum(
      dx_path_proc_cpt_yrs < dx_first_post_met_reg_yrs + 0.5 / 365,
      na.rm = T
    ) >=
      1,
    ddr_before_entry = sum(
      dx_path_proc_cpt_yrs < dx_entry + 0.5 / 365,
      na.rm = T
    ) >=
      1,
    .groups = "drop"
