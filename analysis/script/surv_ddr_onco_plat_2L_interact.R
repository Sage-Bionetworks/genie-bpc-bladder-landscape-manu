...
met_ddr_surv %<>%
  mutate(
    ddr_lab = factor(
      case_when(ddr_onco_alt ~ "DDR+", T ~ "DDR-")
    )
  )
...
model_bundle <- c(model_bundle, list(gg_km_2L = gg_km_2L))
...
met_ddr_surv_mod_ready <- met_ddr_surv %>%
  mutate(
    io = regimen_cat %in% c("IO (atezo/pembro)")
  ) %>%
  select(
    reg_fcpt_yrs,
    tt_os_g_yrs,
...
gg_cox_mod_imp <- ggplot(
  ...
  labs(
    title = "2L model with IO:DDR interaction",
    subtitle = "Multiple imputation used, covariates WIP",
    y = NULL,
