...
first_line_by_ddr <- readr::read_rds(
  file = here('data', 'survival', 'ddr_onco', 'gg_met_ddr_manu_no_rt.rds')
)

first_line_by_ddr_plat <- readr::read_rds(
  file = here('data', 'survival', 'ddr_onco', 'gg_met_ddr_plat_split_no_rt.rds')
)

second_line_by_ddr_io <- readr::read_rds(
  file = here('data', 'survival', 'ddr_onco', 'gg_os_2l_io_ddr_no_rt.rds')
)
