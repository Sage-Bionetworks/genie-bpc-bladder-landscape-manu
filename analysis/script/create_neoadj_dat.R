...
dft_neoadj <- readr::read_csv(
  here('data', 'drug', 'neoadjuvant_regimens_2023_11_06.csv')
)
...
readr::write_rds(
  x = dft_neoadj,
  file = here('data', 'drug', 'neoadjuvant_reg_key.rds')
)
...
dft_reg_neo %<>%
  ...
  mutate(
    tt_met_reg_start = dmet_time_yrs - dx_reg_start_int_yrs,
    tt_met_reg_end = dmet_time_yrs - dx_reg_end_all_int_yrs,
    is_neoadjuvant = case_when(
      ...
      valid_neoadjuvant %in% T & tt_met_reg_start >= -(0.5/365.25) ~ T,
      T ~ F
    )
  )
...
#   mutate(
#     is_gte_6mo = case_when(
      

# Todo:
#  - tag regimens into being neoadjuvant, gte 6 months, lt 6 months.
#  - create a cancer-level dataframe classifying each cancer into the
#      groups laid out in the paper.
