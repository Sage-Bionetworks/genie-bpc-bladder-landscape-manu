...
dft_gp_data_guide <- tribble(
  ...
  "VICC-01-T7", 1, 1,
  "VICC-01-T5A", 1, 1,
  "VICC-01-SOLIDTUMOR", 1, 1, # bit of a guess here too.
  "UHN-48-V1", 0, 0, 
  "UHN-50-V2", 0, 0,
  "UHN-555-V1", 0, 0,
  "UHN-555-BLADDER-V1", 0, 0,
  "UHN-OCA-V3", 0, 1
) %>%
  mutate(
    across(
...
if (length(chk_data_guide_complete)) {
  cli::cli_abort(
    glue("Need to update dft_gp_data_guide to include all represented panels.  Not covered: {paste(chk_data_guide_complete, collapse = ',')}"
    )
  )
}
...
gp_all %<>%
  left_join(
    ., dft_gp_data_guide, by = c(cpt_seq_assay_id = "panel")
  )
