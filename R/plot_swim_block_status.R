...
plot_swim_block_status <- function(
  ...
  if (order_by_label) {
    if (is.null(label_dat)) {
      cli::cli_abort("Cant order by a NULL dataset.")
    }
    ...
    block_dat_sub %<>%
      group_by(record_id) %>%
      mutate(max_fu = max(dx_block_end, na.rm = T)) %>%
      ungroup(.) %>%
      arrange(max_fu) %>%
      mutate(record_id = forcats::fct_inorder(record_id))
  }
  ...
  gg <- ggplot(block_dat_sub, aes(y = record_id, yend = record_id)) +
    geom_segment(
      aes(x = dx_block_start, xend = dx_block_end, color = .data[[block_var]]),
      linewidth = 1
    ) +
    geom_point(
      data = event_dat_sub,
