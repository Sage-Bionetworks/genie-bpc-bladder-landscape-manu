make_obs_count_df <- function(med_onc_dat, img_dat, reg_dat, ca_ind_dat) {
  ...
  dft_count_img <- img_dat %>%
    ...
    pivot_wider(names_from = image_scan_type, values_from = n)
  dft_count_img %<>%
    mutate(
  ...
  dft_count_all <- full_join(
    ...
    full_join(., dft_count_med_onc, by = "record_id") %>%
