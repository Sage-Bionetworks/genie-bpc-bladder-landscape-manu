...
dict_timing_vars <- tribble(
  ...
  "tt_seq_first_reg_yrs", "First NGS (from first regimen)",
...
dict_timing_vars_dmet <- tribble(
  ~raw, ~plot_long,
  "tt_seq_dmet_yrs", "First NGS (from dmet)",
  "tt_reg_post_dmet_yrs", "Post-dmet regimen (from dmet)",
  "tt_pfs_dmet_yrs", "Progression, PFS (from dmet)",
  "tt_os_dmet_yrs", "Followup, OS (from dmet)",
) %>%
  mutate(default = plot_long)
