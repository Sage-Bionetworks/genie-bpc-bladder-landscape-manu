...
plot_timing_intervals <- function(
  ...
  dft_sum <- dat %>%
    ...
    mutate(
      ...
      str_med = glue(
        "Median  (IQR): {form_f(med, ld)} ({form_f(fqt, ld)}, {form_f(tqt, ld)})"
      ),
      str_miss = glue(
        "Missing  (%): {miss_n} ({form_f(miss_prop*100, label_digits)}%)"
      ),
      str_nonneg = glue(
        "Non-negative if obs. (%): {nonneg_n} ({form_f(nonneg_prop*100, label_digits)}%)"
      ),
      str_all = paste(
        str_mean,
        "<br>",
