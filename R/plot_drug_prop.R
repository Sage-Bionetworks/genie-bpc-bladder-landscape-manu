...
plot_drug_prop <- function(
  ...
  gg <- ggplot(data = drug_dat, aes(x = prop, y = drug, fill = drug)) +
    geom_col() +
    annotate(
      geom = 'rect',
      xmin = 1,
    ...
    geom_text(size = 2.5, nudge_x = .05, hjust = 0, aes(label = str)) +
...
plot_drug_prop_vert <- function(
  ...
  gg <- ggplot(data = drug_dat, aes(y = prop, x = drug, fill = drug)) +
    geom_col() +
    labs(title = plot_title) +
    scale_fill_manual(
      values = pal
